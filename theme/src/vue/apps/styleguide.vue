<template>
  <div id="styleguide">
    ...
    <section class="section">
      ...
      <div class="w-full relative h-4 bg-orange-300">
        <div
                v-for="(size, key, index) in theme.screens"
                :key="index"
                class="absolute h-4 mx-auto inset-0"
                :class="'bg-gray-' + (3 + index) + '00'"
                :style="'width:' + size + 'px; z-index:' + (1000 - index)"
              ></div>

      </div>

      <div class="w-full flex sm:hidden bg-gray-300">
        <div class="mx-auto p-4 text-yellow-800 bg-yellow-400">
          {{ getScreen(0) }}
        </div>
      </div>
      ...
      <div class="w-full hidden md:flex lg:hidden bg-gray-500">
        <div class="mx-auto p-4 text-yellow-800 bg-yellow-400">
          {{ getScreen(2) }}
        </div>
      </div>
      <div class="w-full hidden lg:flex xl:hidden bg-gray-600">
        <div class="mx-auto p-4 text-yellow-800 bg-yellow-400">
          {{ getScreen(3) }}
        </div>
      </div>
      <div class="w-full hidden xl:flex 2xl:hidden bg-gray-700">
        <div class="mx-auto p-4 text-yellow-800 bg-yellow-400">
          {{ getScreen(4) }}
        </div>
      </div>
      <div class="w-full hidden 2xl:flex 3xl:hidden bg-gray-800">
        <div class="mx-auto p-4 text-yellow-800 bg-yellow-400">
          {{ getScreen(5) }}
        </div>
      </div>
      <div class="w-full hidden 3xl:flex bg-gray-800">
        <div class="mx-auto p-4 text-yellow-800 bg-yellow-400">
          {{ getScreen(6) }}
        </div>
      </div>
